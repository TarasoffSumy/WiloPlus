  ...
  <div v-loading="loading">
    <div class="lang-container">
      <el-dropdown @command="handleCommand">
        <el-button type="info" class="lang-button">
          {{lang}}
          <i class="el-icon-arrow-down el-icon--right"></i>  
        </el-button>
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item command="RU">RU</el-dropdown-item>
          <el-dropdown-item command="UA">UA</el-dropdown-item>
        </el-dropdown-menu>
      </el-dropdown>
    </div>
    ...
    <div class="container">
      <el-dialog
        ...
        center
      >
        <span class="dialog-text-cookis">
          {{service_dictionary[158]}}
          
          <a
            style="display: inline;"
            class="download"
            href="http://www.wilo.com/en/legal/#.WbaDk9NJalM"
            target="_blank"
            title="legal information"
          >legal information</a>
        </span>
        <span class="dialog-button-cookis">
          <el-button type="primary" @click="onAgreeUseCookies"> {{service_dictionary[159]}}</el-button>
        </span>
      </el-dialog>
      <!-- <div v-loading="loading" > -->
      <!-- {{deliveryHead}}
          ...
          {{selectedPumpId}}
      {{pump}}-->
      <HeaderPart :service_dictionary="service_dictionary"/>
      <el-row :gutter="20">
        <div class="first" @click="step(1, $event)">
          <stepTile :title=service_dictionary[1] number="1" :class="[{ active: current==1}]" />
        </div>
        <div @click="step(2, $event)">
          <stepTile :title=service_dictionary[2]  number="2" :class="[{ active: current==2}]" />
        </div>
        <div @click="step(3, $event)">
          <stepTile :title=service_dictionary[3] number="3" :class="[{ active: current==3}]" />
        </div>
        <div @click="step(4, $event)">
          <stepTile slot="first" :title=service_dictionary[4] number="4" :class="[{ active: current==4}]" />
        </div>
      </el-row>
      <transition name="flip" mode="out-in">
        <Step1
          v-if="current==1 && step1=='ready'"
          :url="url"
        ...
        />
        <Step3
          v-else-if="current==3 && step3=='ready'"
          :url="url"
          :pump="pump"
          :allPumps="allPumps"
          :service_dictionary="service_dictionary"
      ...
      <el-row class="navigation-footer">
        <el-col :span="12">
          ...
          > {{service_dictionary[27]}}</el-button>
        </el-col>
        <el-col :span="12">
          &nbsp;
          <el-button v-show="current != 4" @click="next" type="primary">
            {{service_dictionary[14]}}
...
import { Mixin_ua_dictionary } from '../assets/mixins/Mixin_ua_dictionary.js'
export default {
  mixins: [Mixin_ua_dictionary],
  data() {
    return {
      lang: "",  
      service_dictionary: "",  
  ...
  created: function() {  
    ...
    let rezRegExp = window.location.href.search( /selecttool_ru/i )
    console.log()
    if (rezRegExp >= 1) {
      this.lang="RU";
      this.service_dictionary=this.dictionary_lang_ru;
    }
    else 
    {
      this.lang="UA";
      this.service_dictionary=this.dictionary_lang_uk;
    }    
    this.step1 = "ready";
    ...
    if (this.get_cookie("existPumpOrAccessoreis")) {
      ...
      if (this.get_cookie("idController")) {
        this.postDataGetDetail(
          this.get_cookie("idController"),
          "item1",
          this.get_cookie("controllerQty")
        );
      }
      if (this.get_cookie("idCable")) {
        this.postDataGetDetail(
          this.get_cookie("idCable"),
          "item2",
          this.get_cookie("cableQty")
        );
      }
      if (this.get_cookie("idMufta")) {
        this.postDataGetDetail(
          this.get_cookie("idMufta"),
          "item3",
          this.get_cookie("muftaQty")
        );
      }
      if (this.get_cookie("idVessel")) {
        this.postDataGetDetail(
          this.get_cookie("idVessel"),
          "item4",
          this.get_cookie("vesselQty")
        );
      }
      if (this.get_cookie("idJacket")) {
        this.postDataGetDetail(
          this.get_cookie("idJacket"),
          "item5",
          this.get_cookie("jacketQty")
        );
      }
      if (this.get_cookie("idPump")) {
        this.postDataGetDetail(
          this.get_cookie("idPump"),
          "pump",
          this.get_cookie("pumpQty")
        );
      }
  ...
  mounted() {
     this.coockie_msg=this.service_dictionary[159]
    if (!this.get_cookie("cookies")) {
      this.cokiesDialogVisible = true;
    }
  ...
  methods: {
    ...
    onAgreeUseCookies() {
      document.cookie =
        "cookies=true path=/; expires=Mon, 31 Aug 2025 00:00:00 GMT;";
    ...
    postDataGetDetail: function(id, item, qty) {
      ...
      getPromise.then(response => {
        ...
        if (obj && item == "item1") {
          ...
          this.selectedAccessories[item].price = (
            obj[0].price * this.exchangeRates
          ).toFixed(2);
          this.selectedAccessories[item].current_max =
            obj[0].features.current_max;
          this.selectedAccessories[item].description =
            obj[0].features.description;
        ...
        if (obj && item == "item5") {
          ...
          this.selectedAccessories[item].description =
            obj[0].features.description;
          this.selectedAccessories[item].typeInstallationJackets =
            obj[0].features.installation;
          this.selectedAccessories[item].article = obj[0].article;
          this.selectedAccessories[item].price = (
            obj[0].price * this.exchangeRates
          ).toFixed(2);
    ...
    postDataPump: function(volumeFlow, deliveryHead) {
      ...
      this.loading = true;
      const getPromise = Axios.post(this.url + "db/pumpSelect", {
        volumeFlow: volumeFlow,
        deliveryHead: deliveryHead
      });
      getPromise
    ...
    onGetDataChart(obj) {
      ...
      this.dataChart.WorkPoint = [
        { x: volumeFlow.toFixed(4), y: deliveryHead.toFixed(4) }
      ];
...
.navigation-footer button.el-button.el-button--default {
  background: #363640;
  color: #fff;
  border-radius: 0;
  font-size: 16px;
